...
for id in range(1,5,1):
    save_dir = "C:/Users/fz2co/andes_gym/examples/DDPG_ze_test/trainfreq{}/delay_learning_200 ".format(5*id)
    ## DDPG
    env1 = gym.make('AndesPrimaryFreqControl_rocof-v0')
    ...
    #save rocof
    rocof1 = pd.DataFrame(env1.final_rocof)
    rocof1.to_csv(save_dir + "Finalrocof_ddpg_fix_{}.csv".format(id), index=False)
    rocofBest1 = pd.DataFrame(env1.best_episode_rocof)
    rocofBest1.to_csv(save_dir + "bestrocof_ddpg_sim_{}.csv".format(id), index=False)  
    #min_nadyr
    min_nadyr1 = pd.DataFrame(env1.min_nadyr)
    min_nadyr1.to_csv(save_dir + "nadyr_ddpg_sim_{}.csv".format(id), index=False)    
    ...
    ## TD3       
    env2 = gym.make('AndesPrimaryFreqControl_rocof-v0')
    ...
    time_start = time.time()
    model2.learn(total_timesteps=6000)  # we need to change the total steps with action numbers
    ...
    #save rocof
    rocof2 = pd.DataFrame(env2.final_rocof)
    rocof2.to_csv(save_dir + "Finalrocof_td3_fix_{}.csv".format(id), index=False)
    rocofBest2 = pd.DataFrame(env2.best_episode_rocof)
    rocofBest2.to_csv(save_dir + "bestrocof_td3_sim_{}.csv".format(id), index=False)  
    #min_nadyr
    min_nadyr2 = pd.DataFrame(env2.min_nadyr)
    min_nadyr2.to_csv(save_dir + "nadyr_td3_sim_{}.csv".format(id), index=False)  
    ...
    for i in range(env1.N_Bus):
        ax1.plot(env1.t_render, env1.best_episode_freq[:, i] * 60)
        ax1.plot(env2.t_render, env2.best_episode_freq[:, i] * 60)
    plt.savefig(save_dir + "Bestfreq_ddpg_{}.png".format(id))
    max_best_freq1=(np.max(env1.best_episode_freq,axis=0))*60
    min_best_freq1=(np.min(env1.best_episode_freq,axis=0))*60
    max_final_freq1=(np.max(env1.final_obs_render,axis=0))*60
    min_final_freq1=(np.min(env1.final_obs_render,axis=0))*60
    max_best_freq2=(np.max(env2.best_episode_freq,axis=0))*60
    min_best_freq2=(np.min(env2.best_episode_freq,axis=0))*60
    max_final_freq2=(np.max(env2.final_obs_render,axis=0))*60
    min_final_freq2=(np.min(env2.final_obs_render,axis=0))*60


#### plot ROCOF #####
    fig = plt.figure(figsize=(9, 6))
    ax = fig.add_subplot(1, 1, 1)
    ax.set_xlim(left=0, right=np.max(env1.t_render))
    ...
    for i in range(env1.N_Bus):
        ax.plot(env1.t_render, env1.best_episode_rocof[:, i]*60)
        ax.plot(env2.t_render, env2.best_episode_rocof[:, i]*60)
    plt.savefig(save_dir + "Bestrocof_ddpg_{}.png".format(id))
    max_best_rocof1=(np.max(env1.best_episode_rocof,axis=0))*60
    min_best_rocof1=(np.min(env1.best_episode_rocof,axis=0))*60
    max_final_rocof1=(np.max(env1.final_rocof_render,axis=0))*60
    min_final_rocof1=(np.min(env1.final_rocof_render,axis=0))*60
    max_best_rocof2=(np.max(env2.best_episode_rocof,axis=0))*60
    min_best_rocof2=(np.min(env2.best_episode_rocof,axis=0))*60
    max_final_rocof2=(np.max(env2.final_rocof_render,axis=0))*60
    min_final_rocof2=(np.min(env2.final_rocof_render,axis=0))*60

    fre_rocof_eva=[]
    fre_rocof_eva.append(max_final_freq1)
    fre_rocof_eva.append(min_final_freq1)
    fre_rocof_eva.append(max_best_freq1)
    fre_rocof_eva.append(min_best_freq1)
    fre_rocof_eva.append(max_final_rocof1)
    fre_rocof_eva.append(min_final_rocof1)
    fre_rocof_eva.append(max_best_rocof1)
    fre_rocof_eva.append(min_best_rocof1)
    fre_rocof_eva.append(max_final_freq2)
    fre_rocof_eva.append(min_final_freq2)
    fre_rocof_eva.append(max_best_freq2)
    fre_rocof_eva.append(min_best_freq2)
    fre_rocof_eva.append(max_final_rocof2)
    fre_rocof_eva.append(min_final_rocof2)
    fre_rocof_eva.append(max_best_rocof2)
    fre_rocof_eva.append(min_best_rocof2)
