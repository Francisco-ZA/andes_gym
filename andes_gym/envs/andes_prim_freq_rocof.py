...
class AndesPrimaryFreqControl_rocof(gym.Env):
    """
    Load frequency control environment using ANDES.
    ...
    def __init__(self):
        ...
        self.path = os.path.join(path, "ieee14_alter_pq_IEESGORM1.xlsx")  #zp

        self.tf = 30.0     # end of simulation time
        self.tstep = 1/30  # simulation time step zp:andex Fixed step size: h=33.33 ms. Shrink if not converged, actually does not change if only change here
        # self.tf = 30.0     # end of simulation time
        ...
        # self.action_instants = np.linspace(1, 100, 100)
        ...
        # self.observation_space = spaces.Box(low=-0.2, high=0.2, shape=(self.N_Gov,)) #obs is freq
        self.observation_space = spaces.Box(low=-0.2, high=0.2, shape=(2*self.N_Gov,)) #obs are freq and rocof
        ...
        self.final_rocof_render = None #zp
    ...
    def initialize(self):
        ...
        #To apply a PQ load change, according to PQ, one needs to set the load model to constant power and alter Ppf and Qpf.
        self.disturbance = 0.1 ## random.uniform(0.2, 0.5) # load change 0.1 p.u. at t=0.1s
        # self.disturbance = random.uniform(0.2, 0.5)
        self.sim_case.Alter.amount.v[0] = self.disturbance
        # self.sim_case.Alter.t.v[0] = 1 #disturbance time  original t = 0.1s
    ...
    def reset(self):
        ...
        self.rocof_print.append(rocof[0]) #zp 
        #return is the obs, should be changed
    ...
    def step(self, action):
        ...
        if self.i < 10:
            coordsig=action
            # coordsig = np.zeros(self.N_Gov)  #no action
        ...
        if not sim_crashed and done:
            reward -= np.sum(np.abs(3000 * (freq - .994712453)))  # the final episode
        else:
...
######### reward generated by rocof  ##########
        ...
        if done:
            ...
            self.final_rocof.append(self.rocof_print[-1]*60)  #zp
            # print('#############################')
            # print(self.final_rocof)
            ...
            # self.sim_case.dae.ts.unpack(df=True)
            ...
            self.final_obs_render = np.array(ydata)  #frq
            self.final_rocof_render = np.array(zdata) #zp rocof
            # print(self.final_obs_render[:,0])
            nadyr=(np.min(self.final_obs_render[:,0],axis=0))*60
            self.min_nadyr.append(nadyr) #ze
            # xx=input("hi")
            ...
            if sum(self.reward_print) > self.best_reward:
                ...
                self.best_episode_rocof = self.final_rocof_render #zp
...
#         """
#         path = pathlib.Path(__file__).parent.absolute()
#         self.path = os.path.join(path, "ieee14_alter_pq_IEESGORM1.xlsx")  #zp #ze
...
#         self.action_instants = np.linspace(1, 30, 30) #ze
...
#         self.initialize()
#         freq = self.sim_case.dae.x[self.w]
#         rocof = np.array(self.sim_case.dae.y[self.dwdt]).reshape((-1, )) #ze
#         self.freq_print.append(freq[0])
#         self.rocof_print.append(rocof[0]) #zp 
#         obs = np.append(freq, rocof)
#         return obs
